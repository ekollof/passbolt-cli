...
class PassboltAuth:
    ...
    def get_auth_token(self) -> requests.Session:
        ...
        try:
            ...
            clean_fingerprint = self.fingerprint.replace('-', '').replace(' ', '').upper()
            
            # Stage 1: Request login challenge (POST to /auth/login.json)
            login_url = urljoin(self.config.server_url, '/auth/login.json')
            ...
            if 'X-GPGAuth-User-Auth-Token' not in response.headers:
                ...
                raise ValueError(
                    f"No encrypted token received in X-GPGAuth-User-Auth-Token header.\n"
            ...
            if authenticated.lower() != 'true':
                ...
                raise ValueError(
                    f"Authentication not confirmed (X-GPGAuth-Authenticated != true).\n"
